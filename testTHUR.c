    ...
    if (Py_IsInitialized()) {                                               // Python 인터프리터 초기화 확인 
        ...
        if (PyCallable_Check(pFunc)) {                                      
            ...
            for (int i = 1; i < 11; i++) {                            
                ...
                bigint* B = NULL;
                bigint_gen_rand(&A, rand() & 0x01, 40);
                bigint_gen_rand(&B, rand() & 0x01, 39);
                while (IsZero(B)) {
                    bigint* B_ = NULL;
                    bigint_gen_rand(&B_, rand() & 0x01, 1);
                    bigint_assign(&B, B_);
                    bigint_delete(&B_);
                }
                bigint* Q = NULL;
...
{
    PyObject* pName, * pModule, * pFunc, * pValue, * pA, * A_sign, * pN, * N_sign, * pB, * B_sign, *pC,*C_sign,*pM, *M_sign;
    Py_Initialize();
    if (Py_IsInitialized()) {                                               // Python 인터프리터 초기화 확인 
#if WordBitLen == 8
        pName = PyUnicode_FromString("test_8bit_version");                              // Python File name -> test1.py, 초기화 필요! 
...
#endif
        pModule = PyImport_Import(pName);                                   // Import Python File            초기화 필요!
        pFunc = PyObject_GetAttrString(pModule, "Modular_exponentiation1"); 
        pFunc = PyObject_GetAttrString(pModule, "Modular_exponentiation2");    // test1.py에 있는 add 함수 (= 함수 이름)  초기화 필요!
        if (PyCallable_Check(pFunc)) {                                    
            printf("Modular Exponentiation 1,2 Testing\n");
            printf("Progress : ");
            for (int i = 1; i < 11; i++) {                           
                bigint* A = NULL;
                bigint* N = NULL;
                bigint* M = NULL;
                bigint_gen_rand(&A, rand() & 0x01, 1);
                bigint_gen_rand(&N, rand() & 0x01, 1);
                bigint_gen_rand(&M, rand() & 0x01, 1);
                while (IsZero(M)) {
                    bigint* M_ = NULL;
                    bigint_gen_rand(&M_, rand() & 0x01, 1);
                    bigint_assign(&M, M_);
                    bigint_delete(&M_);
                }
                bigint* B = NULL;
                ...
                unsigned char* string_C = array2string(C);
                pC = PyUnicode_FromString(string_B);
                C_sign = Py_BuildValue("i", C->sign);
                ...
                // A, A_sign, B, B_sign, sum, sum_sign
                pValue = PyObject_CallFunctionObjArgs(pFunc, pA, A_sign, pN, N_sign, pB, B_sign,pC,C_sign, pM, M_sign, NULL);     // 파이썬 mul_k 함수 실행 ( 매개 변수 여러 개 전달 ) 초기화 필요!
                if (!PyLong_AsLong(pValue) || PyLong_AsLong(pValue) == -1) {
                    printf("\nError");
                    printf("\n");
