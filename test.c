...
{   
    ...
    PyObject *pName, *pModule, *pFunc, *pValue, *pArg1, *pArg2, *pArg3, *pArg4;
    ...
    if (Py_IsInitialized()) {                                               // Python 인터프리터 초기화 확인 
        ...
        if (PyCallable_Check(pFunc)) {                                      // 함수가 잘 불러져 왔는지 확인
            ...
            for (int i = 1; i < 100001; i++) {                               // 100000번 테스트
                ...
                bigint* B = NULL;
                bigint_gen_rand1(&B, NON_NEGATVE, 17);
                ...
                pValue = PyObject_CallFunctionObjArgs(pFunc, pArg1, pArg2, pArg3, pArg4, NULL);     // 파이썬 mul_k 함수 실행 ( 매개 변수 여러 개 전달 ) 초기화 필요!
